...
<!-- <input type="text" placeholder="Search by Location.." id="search2" oninput = "onclick()" name = "search2" id="search2" type="submit"> -->
  ...
  <script>
    ...
    var mark = function() {
      // Read the keyword
      var keyword = $("input[name='<?php echo $query?>']").val();
      ...
      'body' => [
       ...
       'size' => 10000,
          'query' => [
              'multi_match' => [
               'query' => $query,
               'fields'=> ['address','neighborhood','council_district','pin']
              ]
          ]
      ]
  ]; 
...
​function highlight(text) {
  var inputText = document.getElementById("'.$results[$i]['_id'].'");
...
​</script>
<button onclick="highlight('<php $query ?>')">Highlight</button> -->
</body>
</html>
